...
date: 'created 2022-10-01, updated `r Sys.Date()`'
output: 
  html_document:
    toc: true
    toc_float: true
...
# The dataset is not totally clean. Issues include a lot of NAs in the 'Code' column, column name with many spaces and possibly issues with multiple nations listed in one row for the 'Entity' column.

# 'Entity' represents the country
# 'Code' represents the country code
# 'Year' is the year of the homeside rate
# 'Homicide rate in Europe over long-term (per 100,000)' represnts how many homicides there were per 100,000.

# The difference between homicide rate and number is that the number is an absolute figure whereas a rate takes into account how many people there were at that year in total.
```
...
ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k, color = Entity)) + 
  geom_smooth() +
  ggtitle("Homicide Rates throughout time for select countries") +
  xlab("Year") +
  ylab("Homicides per 100k People") +
...
# create gg-plot
ggplot(kings, aes(x = midyear, y = as.numeric(Yearasruler))) +
  geom_smooth() +
  ggtitle("Reign Durations for Danish Kings over Time") +
  xlab("Time of Reign") +
  ylab("Reign Duration") +
  theme_bw()
...
The data visualisations suggest that we are more civilized today. Based on the facet-wrap plot, we can tell that homicide rates have been declining rapidly in all select countries since the 14th ceentury. For instance, Italy starts at almots 60 homicides per 100k people in the early 14th century, but this then steadily declines all the way to today. In fact, the lines for all countries almost go down to the x-axis which suggests that homicides barely happen in these European countries in the 21st century. However, as to whether we are more civilized, it is naturally based on the assumption that populations with lower homicide rates are more civilized. There are certainly more variables which could be taken into the equation in order to answer that question more holistically. 
